...
class Config:
    ...
    MONTE_CARLO_RUNS = 5000        # Max accuracy
    MIN_LAPS_FOR_STINT = 4         
    OUTLIER_THRESHOLD = 1.07       
    CACHE_DIR = 'f1_cache'
    
    # RELAXED ANOMALY SETTINGS
    ANOMALY_DIFF_LIMIT = 2.0       # Was 1.2 -> Now 2.0 (Allows dominance)
    
    SC_CHANCE = {
        "Monaco": 0.8, "Singapore": 1.0, "Italy": 0.4, "DEFAULT": 0.3
    }
...
# ==========================================
# 3. PHYSICS ENGINE (SMART ANOMALY FIX)
# ==========================================
class TelemetryAnalyzer:
    ...
    def load_data(self):
        try:
            ...
            if self.session_type == 'FP2':
                rprint("[yellow]HINT: Try using '--session FP1'.[/yellow]")
            return None
    ...
    def analyze_stints(self, session):
        ...
        # --- SMART ANOMALY FILTER (V7 FIX) ---
        if raw_profiles:
            # 1. Get all paces
            all_paces = sorted([p['base_pace'] for p in raw_profiles.values()])
            
            if all_paces:
                # 2. Calculate Baseline from TOP 50% only (Ignore backmarkers)
                midpoint = len(all_paces) // 2
                top_half = all_paces[:midpoint+1] # Faster half
                elite_median = np.median(top_half)
                
                # 3. Define Threshold (Elite Median - 2.0s)
                # This allows for a dominant car, but catches "Impossible" 3s gaps
                threshold = elite_median - Config.ANOMALY_DIFF_LIMIT
                
                for abbr, data in raw_profiles.items():
                    if data['base_pace'] < threshold:
                        diff = elite_median - data['base_pace']
                        rprint(f"[yellow]⚠ ANOMALY: {abbr} is {diff:.2f}s faster than Elite Median ({elite_median:.2f}s).[/yellow]")
                        
                        # 4. SOFT CLAMP: Set to the 2nd fastest VALID time (or Elite Median - 0.5)
                        # This keeps them winning, but removes the "broken" advantage
                        data['base_pace'] = elite_median - 0.5
...
def main():
    ...
    console.print(Panel.fit(f"F1 PREDICTOR V7 (ELITE MEDIAN FIX) | {args.year} {args.gp}", style="bold red"))
    ...
    if args.session in ['FP1', 'FP2']:
        ...
        table = Table(title=f"Race Pace Analysis (Elite Median)")
